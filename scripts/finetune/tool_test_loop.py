...
_PROMPT_RE = re.compile(r"`([^`]+)`\s*(?:\([^)]*\)\s*)?(?:→|->)\s*\*\*([a-z][a-z0-9_.]+)\*\*", re.I)
...
def execute_call(name: str, args: dict) -> dict:
    ...
    if name not in READ_ONLY:
        return {
            "output": f"[mock] {name} not executed (off the read-only allowlist)",
            "success": True,
            "mocked": True,
        }
    try:
        ...
        trace_msgs.append(
            {
                "role": "tool",
                "content": json.dumps(
                    {"output": str(outcome["output"])[:4000], "success": outcome["success"]}
                ),
            }
        )
    else:
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"targets: {len(targets)} tools with seed prompts"
    ...
    for t in targets:
        ...
        for phrasing in phrasings:
            for _ in range(args.repeats):
                ...
                print(
                    f"  {mark} {t.tool:26} fired={r.fired or '—'}"
    ...
    (out_dir / "scorecard.json").write_text(
        json.dumps(
            {"tools": rows, "totals": dict(agg), "elapsed_s": round(time.time() - t0, 1)}, indent=2
        ),
        encoding="utf-8",
    )
    ...
    print(
        "Next: python scripts/finetune/verify_examples.py "
