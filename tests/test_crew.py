...
def test_boss_when_tools_open_but_nothing_matched():
    ...
    r = triage(
        tools_open=True, keyword_gated=True, needs_think=False, category_scores=[("network", 0.30)]
    )
...
def test_boss_when_index_unavailable_and_keyword_gated():
    ...
    r = triage(tools_open=True, keyword_gated=True, needs_think=False, category_scores=[])
    assert r.profile is Profile.BOSS
...
def test_crew_turn_fast_injects_findings():
    ...
    decision = crew.TriageResult(
        profile=crew.Profile.FAST, specialist="Gus", think=False, tools=True
    )
...
def test_crew_turn_chat_runs_nothing():
    ...
    decision = crew.TriageResult(
        profile=crew.Profile.REASON, specialist=None, think=True, tools=False
    )
    messages, tools_used, think = _crew_turn(brain, decision)
...
def test_semantic_think_axis_sets_reason():
    ...
    think = _drain(
        brain._crew.run_turn(
            "walk me through the tradeoffs here",
            messages,
            tools_used,
            query_emb=[0.0] * 384,
            handoff_mode="chat",
