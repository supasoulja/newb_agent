...
def _client():
    return OpenAIClient(
        api_key="sk-test", base_url="https://api.example.com/v1", default_model="gpt-4o-mini"
    )
...
def test_chat_normalizes_content(monkeypatch):
    c = _client()
    monkeypatch.setattr(
        c, "_post_json", lambda p, payload: {"choices": [{"message": {"content": "hi there"}}]}
    )
...
def test_installed_models(monkeypatch):
    c = _client()
    monkeypatch.setattr(
        c, "_get_json", lambda p: {"data": [{"id": "gpt-4o"}, {"id": "gpt-4o-mini"}]}
    )
...
def test_resolve_cloud_entry_with_key(fixed_device_key):
    keystore.set_key(
        5, "openrouter", "sk-live", provider="openai", base_url="https://openrouter.ai/api/v1"
    )
    entry = {
        "provider": "openai",
