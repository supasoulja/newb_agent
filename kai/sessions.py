...
def new_session(title: str) -> str:
    ...
    conn = get_conn()
    conn.execute(
        "INSERT INTO sessions (id, title, started_at, last_active, message_count) "
        "VALUES (?, ?, ?, ?, 0)",
        (sid, title[:60], now, now),
    )
    conn.commit()
    return sid
...
def append_message(
    ...
    conn = get_conn()
    cur = conn.execute(
        "INSERT INTO session_messages (session_id, role, content, timestamp, turn_order) "
        "VALUES (?, ?, ?, ?, ?)",
        (session_id, role, content, now, turn_order),
    )
    conn.execute(
...
def search_messages(query: str, limit: int = 10) -> list[dict]:
    """
    Search across ALL past session messages by keyword.
