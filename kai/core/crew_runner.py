...
from kai.core import crew, crew_trace
...
class CrewRunner:
        ...
        *,
        ...
        if query_emb is not None:
            try:
                ...
                pass
        needs_think = _query_needs_thinking(user_input) or handoff_mode == "reasoning" or think_sem
        ...
        decision = self.triage(
            user_input,
            query_emb,
            tools_open=tools_open or tool_sem,
            keyword_gated=tools_open,
        ...
        flow_rec.record(
            ...
            specialist=decision.specialist or "-",
            lane=decision.lane,
    ...
    def run_specialist(
        ...
        *,
        ...
        slice_names = (
            crew.tools_for_specialist(name, self._brain.tool_registry.category_tool_map())
            if self._brain.tool_registry
            else []
        )
        ...
        *,
        ...
        for _step in range(crew.MAX_DISPATCHES):
            ...
            if forced:
                ...
                coverage_fired += 1
                flow_rec.record(
                    trace_id, "coverage_dispatch", specialist=specialist, remaining=len(missing)
                )
                crew_trace.record(
                    trace_id,
                    "coverage_dispatch",
    ...
    def _otto_decide(
        ...
        try:
            tool_model = (
                want
                if want in self._brain.ollama.installed_models()
                else self._brain._engine._resolve_tool_model()[0]
            )
        except Exception:
        ...
        if tool_model is None:
            resp = self._brain._engine._chat(
                messages, think=False, temperature=cfg.TEMPERATURE_TOOL
            )
        else:
        ...
        flow_rec.record(
            trace_id, "otto", text=text, decision="/".join(d for d in (decision or ("none",)) if d)
        )
        return decision
