...
def shutdown_reembed(progress_cb=None) -> None:
    ...
    progress_cb, if given, is called as progress_cb(phase, detail, pct) so a
    graceful-shutdown UI can show "finishing embeddings" progress. Optional —
    the function works identically without it.
    ...
    def _tick(detail: str = "", pct: int | None = None) -> None:
        if progress_cb:
            try:
                progress_cb("hq-reembed", detail, pct)
            except Exception:
                pass
    ...
    if rows:
        ...
        if to_embed:
            ...
            _tick(f"{len(to_embed)} episodic entries", 0)
    ...
    if rag_rows:
        ...
        if to_embed_rag:
            ...
            _tick(f"{len(to_embed_rag)} document chunks", 0)
