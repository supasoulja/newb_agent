...
def select_for_context(
    ...
    hardcoded = [n for n in _tree.hardcoded_nodes(user_id)
                 if n.source != "seed"]                # always-surface nodes, unscored
    ...
    rest = [n for n in _tree.all_nodes(user_id)
            if n.path not in hardcoded_paths and n.source != "seed"]
    ...
    ranked = rank_nodes(rest, query_embedding, active_domains, context_modifier, top_k=remaining_slots)
    ...
    result: list[tuple[Node, "Optional[float]"]] = [(n, None) for n in hardcoded]
