...
def _ps(cmd: str, timeout: int = 10) -> str:
    ...
    return run_powershell(cmd, timeout).out
...
@registry.tool(
    ...
    description=(
        ...
        "report the load and clock data that IS available, then suggest HWiNFO64 (Windows) "
        "or lm-sensors (Linux) for detailed sensor readings. Do NOT say the tool failed."
    ),
)
def get_temps() -> str:
    if not _IS_WINDOWS:
        return _gather_temps_linux()
...
def _gather_temps_linux() -> str:
    lines = []

    # CPU temps via psutil.sensors_temperatures() — reads /sys/class/hwmon directly
    try:
        ...
        if cpu_temps:
            ...
            lines.append(
                f"CPU: Temp: {max(cpu_temps):.0f}°C  Load: {cpu_pct:.0f}%  Clock: {freq_str}"
            )
        elif all_temps:
    ...
    # GPU: AMD first (amd-smi/rocm-smi/hwmon), then NVIDIA
    gpu = _amd_smi() or _rocm_smi() or _amd_hwmon_linux() or _nvidia_smi()
    if gpu:
        lines.append(gpu)
    ...
    if not lines:
        return (
            ...
            "For CPU temps: sudo apt install lm-sensors && sudo sensors-detect\n"
            "For GPU temps (NVIDIA): nvidia-smi should be installed with drivers.\n"
            "For GPU temps (AMD): install ROCm — amd-smi or rocm-smi"
        )
...
def _amd_hwmon_linux() -> str | None:
    ...
    try:
        ...
        for name_file in glob.glob("/sys/class/hwmon/hwmon*/name"):
            ...
            # Fan
            fan = _read(f"{hwmon_dir}/fan1_input")
            fan_str = f"{fan} RPM" if fan else "n/a"
            ...
            if slot:
                try:
                    lspci = subprocess.run(
                        ["lspci", "-s", slot], capture_output=True, text=True, timeout=3
                    )
                    desc = lspci.stdout.strip()
                    if ":" in desc:
                        gpu_name = desc.split(":", 2)[-1].strip()
                except Exception:
                    pass
            ...
            lines.append(
                f"GPU: {gpu_name} ({chip})\n"
                f"  Temps: {', '.join(temp_parts) or 'n/a'}\n"
                f"  Load: {load_str}  Fan: {fan_str}  Power: {pwr_str}"
            )
